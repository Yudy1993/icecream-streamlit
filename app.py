...
st.set_page_config(page_title="🍦 Inpay's Icecream Shop", layout="centered")

# Auto-start loop when app runs
if "looping" not in st.session_state:
    st.session_state.looping = True  # Auto-start enabled

st.title("🍨 Inpay's Icecream Shop")

# Main loop
if st.session_state.looping:
    img_url = fetch_ice_cream_image()
    if img_url:
        st.image(img_url, caption="Data & AI LOVES Icecream", use_container_width=True)
    else:
